本项目基于dnmp项目的二次的补充, 集成大量php扩展内置包和其他配置等环境调整
...
- [目录](#目录)
  ...
  - [8 Docker使用cron定时任务](#8-docker使用cron定时任务)
  - [9 Docker容器时间](#9-docker容器时间)
    - [8.4 如何连接MySQL和Redis服务器](#84-如何连接mysql和redis服务器)
    - [8.4 docker容器访问宿主机方式](#84-docker容器访问宿主机方式)
  - [License](#license)

## 鸣谢
> 非常感谢[**[yeszao]**](https://github.com/yeszao)作者的[**[DNMP]**](https://github.com/yeszao/dnmp)项目

## 项目的特点：
1. `100%`开源
2. `100%`遵循Docker标准
3. 支持**多版本PHP**共存，可任意切换（PHP5.4、PHP5.6、PHP7.1、PHP7.2、PHP7.3、PHP7.4)
4. 支持绑定**任意多个域名**
5. 支持**HTTPS和HTTP/2**
...
2. `clone`项目：
    ```
    $ git clone https://github.com/qklandy/phper-docker
    ```
...
4. 拷贝并命名配置文件（Windows系统请用`copy`命令），启动：
    ...
    $ docker-compose up -d php74 mysql80 openresty      # 选择性启动
    ```
...
#### 3.2.1 容器内扩展安装
参考[**[mlocati/docker-php-extension-installer]**](https://github.com/mlocati/docker-php-extension-installer)的扩展支持
```
...
### 3.4 使用composer
**方法1：主机中使用composer命令**
1. 确定composer缓存的路径。比如，我的phper-docker下载在`~/phper-docker`目录，那composer的缓存路径就是`~/phper-docker/data/composer`。
2. 参考[bash.alias.sample](bash.alias.sample)示例文件，将对应 php composer 函数拷贝到主机的 `~/.bashrc`文件。
    > 这里需要注意的是，示例文件中的`~/phper-docker/data/composer`目录需是第一步确定的目录。
3. 让文件起效：
    ```bash
    source ~/.bashrc
    ```
4. 在主机的任何目录下就能用composer了：
    ...
    ```
5. （可选）第一次使用 composer 会在 `~/phper-docker/data/composer` 目录下生成一个**config.json**文件，可以在这个文件中指定国内仓库，例如：
    ```json
    {
        "config": {},
        "repositories": {
